...
func TestGetData_SerializationDeserialization(t *testing.T) {
	...
	if len(originalData) == 0 {
		t.Fatal("GetData() returned empty map, expected country metadata data")
	}
	...
	root := struct {
		...
		MetadataInformation: originalData,
	}
	...
	if len(deserializedRoot.MetadataInformation) != len(originalData) {
		t.Errorf("Deserialized data count = %d, want %d", 
			len(deserializedRoot.MetadataInformation), len(originalData))
	}
	
	// Verify that all keys from original exist in deserialized and vice versa
	for key := range originalData {
		if _, exists := deserializedRoot.MetadataInformation[key]; !exists {
			t.Errorf("Key %q exists in original but not in deserialized data", key)
		}
	}
	
	for key := range deserializedRoot.MetadataInformation {
	...
	for _, key := range sampleKeys {
		...
		deserialized := deserializedRoot.MetadataInformation[key]
