...
func TestGetData_SerializationDeserialization(t *testing.T) {
	...
	if len(originalData) == 0 {
		t.Fatal("GetData() returned empty map, expected phone-number data")
	}
	...
	root := struct {
		...
		CountryTeleInformation: originalData,
	}
	...
	if len(deserializedRoot.CountryTeleInformation) != len(originalData) {
		t.Errorf("Deserialized data count = %d, want %d", 
			len(deserializedRoot.CountryTeleInformation), len(originalData))
	}
	
	// Verify that all keys from original exist in deserialized and vice versa
	for key := range originalData {
		if _, exists := deserializedRoot.CountryTeleInformation[key]; !exists {
			t.Errorf("Key %q exists in original but not in deserialized data", key)
		}
	}
	
	for key := range deserializedRoot.CountryTeleInformation {
	...
	for _, key := range sampleKeys {
		...
		deserialized := deserializedRoot.CountryTeleInformation[key]
