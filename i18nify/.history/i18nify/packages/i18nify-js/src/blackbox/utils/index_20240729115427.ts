  ...
  // using try catch blocks to ensures error handling and exception cases 
    ...
    try{
      ...
      const amount = eval(${JSON.stringify(safeScript)})
       ...
       /yup their might be useing eval can execute any kind of js code in dynamic execution lead to malicious input highly dangerous inputs   
// why we have using JSON.stringify method cause it converts JSON objects to strings where we cna embed the dynamic content or data into our scripts where the data should be properly formatted show it cna embed the data into our html contet properly 
