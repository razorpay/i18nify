...
func MaskCardNumber(cardNumber string, opts *MaskCardOptions) (string, error) {
	if strings.TrimSpace(cardNumber) == "" {
		return "", fmt.Errorf("masking: MaskCardNumber: cardNumber must be a non-empty string")
	}
	...
	for _, ch := range cardNumber {
		...
		if !unicode.IsDigit(ch) {
			return "", fmt.Errorf("masking: MaskCardNumber: cardNumber may only contain digits, spaces, or hyphens")
		}
		digits.WriteRune(ch)
	}
	d := digits.String()
