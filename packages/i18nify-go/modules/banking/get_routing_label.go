...
// RoutingCodeLabel describes a payment routing code identifier.
// Country is an ISO 3166-1 alpha-2 code and is empty for international codes.
...
// routingLabelMap maps routing code type keys to their display labels.
...
// GetRoutingLabel returns display label information for the given payment routing
// code type (e.g. "IFSC", "SWIFT", "IBAN"). Lookup is case-insensitive.
func GetRoutingLabel(routingCodeType string) (RoutingCodeLabel, error) {
	if strings.TrimSpace(routingCodeType) == "" {
		return RoutingCodeLabel{}, fmt.Errorf("banking: GetRoutingLabel: routingCodeType must be a non-empty string")
	}
	label, ok := routingLabelMap[strings.ToUpper(strings.TrimSpace(routingCodeType))]
	if !ok {
		return RoutingCodeLabel{}, fmt.Errorf("banking: GetRoutingLabel: routing code type %q is not supported", routingCodeType)
	}
	return label, nil
}
