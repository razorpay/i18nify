...
func GetMandateFrequencyLabel(frequencyCode string) (MandateFrequencyLabel, error) {
	if strings.TrimSpace(frequencyCode) == "" {
		return MandateFrequencyLabel{}, fmt.Errorf("mandate: GetMandateFrequencyLabel: frequencyCode must be a non-empty string")
	}
	label, ok := frequencyLabelMap[strings.ToUpper(strings.TrimSpace(frequencyCode))]
	if !ok {
		return MandateFrequencyLabel{}, fmt.Errorf("mandate: GetMandateFrequencyLabel: frequency code %q is not supported", frequencyCode)
	}
	return label, nil
}
