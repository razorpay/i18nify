...
func GetCountryTaxDefinition(countryCode string) (CountryTaxDefinition, error) {
	if strings.TrimSpace(countryCode) == "" {
		return CountryTaxDefinition{}, fmt.Errorf("tax: GetCountryTaxDefinition: countryCode must be a non-empty string")
	}
	def, ok := taxDefinitions[strings.ToUpper(strings.TrimSpace(countryCode))]
	if !ok {
		return CountryTaxDefinition{}, fmt.Errorf("tax: GetCountryTaxDefinition: country code %q is not supported", countryCode)
	}
	return def, nil
}
