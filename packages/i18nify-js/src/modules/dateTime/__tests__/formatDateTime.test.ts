...
describe('dateTime - formatDateTime', () => {
  ...
  test.each([
    ...
    (date, locale, options, expected) => {
      expect(formatDateTime(date, locale, options)).toBe(expected);
    },
  );

  test('formats end of year date with time', () => {
    expect(formatDateTime('2024-12-31T23:59:59', 'en-US')).toBe(
      '12/31/2024, 11:59:59 PM',
    );
  });

  test('handles invalid date strings', () => {
    expect(() => formatDateTime('invalid-date', 'en-US')).toThrow();
  });

  // Locale and Option Variations
  test('formats date and time with different locales', () => {
    const date = '2024-03-01T20:00:00';
    expect(formatDateTime(date, 'fr-FR')).not.toBe(
      formatDateTime(date, 'de-DE'),
    );
  });
  ...
  test('formats date and time with different options', () => {
    ...
    expect(formatDateTime(date, 'en-US', options1)).not.toBe(
      formatDateTime(date, 'en-US', options2),
    );
  });
});
