...
describe('dateTime - formatDateTime', () => {
  ...
  describe('locale date ordering', () => {
    test('formats date in YMD order for ja-JP locale', () => {
      const result = formatDateTime('2024-03-15', {
        locale: 'ja-JP',
        dateTimeMode: 'dateOnly',
      });
      // ja-JP produces year/month/day ordering
      expect(result).toMatch(/2024/);
    });

    test('formats date in DMY order for en-GB locale', () => {
      expect(
        formatDateTime('2024-03-05', {
          locale: 'en-GB',
          dateTimeMode: 'dateOnly',
        }),
      ).toBe('05/03/2024');
    ...
    test('formats date in MDY order for en-US locale', () => {
      expect(
        formatDateTime('2024-03-05', {
          locale: 'en-US',
          dateTimeMode: 'dateOnly',
  ...
  describe('field style options', () => {
    test('formats month as long name', () => {
      const result = formatDateTime('2024-03-01', {
        locale: 'en-US',
    ...
    test('formats month as short name', () => {
      const result = formatDateTime('2024-03-01', {
        locale: 'en-US',
        intlOptions: { month: 'short', day: 'numeric', year: 'numeric' },
      });
      expect(result).toContain('Mar');
    });

    test('formats day as 2-digit', () => {
      const result = formatDateTime('2024-03-01', {
        locale: 'en-US',
