...
describe('dateTime - getRelativeTime', () => {
  ...
  describe('baseDate option', () => {
    const BASE = new Date('2024-06-01T12:00:00Z');

    test('calculates relative time from a fixed baseDate (1 day ago)', () => {
      const target = new Date('2024-05-31T12:00:00Z');
      expect(getRelativeTime(target, { baseDate: BASE })).toBe('1 day ago');
    });

    test('calculates future relative time from a fixed baseDate (2 days ahead)', () => {
      const target = new Date('2024-06-03T12:00:00Z');
      expect(getRelativeTime(target, { baseDate: BASE })).toBe('in 2 days');
    });

    test('calculates relative weeks from a fixed baseDate', () => {
      const target = new Date('2024-05-18T12:00:00Z'); // ~2 weeks before base
      expect(getRelativeTime(target, { baseDate: BASE })).toBe('2 weeks ago');
    });

    test('calculates relative months from a fixed baseDate', () => {
      const target = new Date('2024-03-01T12:00:00Z'); // ~3 months before base
      expect(getRelativeTime(target, { baseDate: BASE })).toBe('3 months ago');
    });

    test('calculates relative years from a fixed baseDate', () => {
      const target = new Date('2022-06-01T12:00:00Z'); // 2 years before base
      expect(getRelativeTime(target, { baseDate: BASE })).toBe('2 years ago');
    });
  });

  describe("numeric: 'always' option", () => {
    const BASE = new Date('2024-06-01T12:00:00Z');

    test('forces numeral output for 1 day (suppresses "yesterday")', () => {
      const target = new Date('2024-05-31T12:00:00Z');
      expect(
        getRelativeTime(target, {
          baseDate: BASE,
          intlOptions: { numeric: 'always' },
        }),
      ).toBe('1 day ago');
    });

    test('forces numeral output for 1 week (suppresses "last week")', () => {
      const target = new Date('2024-05-25T12:00:00Z');
      expect(
        getRelativeTime(target, {
          baseDate: BASE,
          intlOptions: { numeric: 'always' },
    ...
    test('forces numeral output for 1 month (suppresses "last month")', () => {
      const target = new Date('2024-05-01T12:00:00Z');
      expect(
        getRelativeTime(target, {
          baseDate: BASE,
          intlOptions: { numeric: 'always' },
