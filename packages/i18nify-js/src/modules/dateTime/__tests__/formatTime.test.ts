...
describe('formatTime function', () => {
  ...
  test.each([
    ...
    (date, locale, options, expected) => {
      expect(formatTime(date, locale, options)).toBe(expected);
    },
  );

  test('formats midnight time', () => {
    expect(formatTime('2024-01-01T00:00:00', 'en-US')).toBe('12:00:00 AM');
  });

  test('formats end of day time', () => {
    expect(formatTime('2024-01-01T23:59:59', 'en-US')).toBe('11:59:59 PM');
  });
  ...
  test('formats time with different options', () => {
    ...
    expect(formatTime(date, 'en-US', options1)).not.toBe(
      formatTime(date, 'en-US', options2),
    );
  });
});
