...
describe('geo - getListOfAllFlags', () => {
  it('should correctly map country codes to their flag SVG content', () => {
    ...
    Object.keys(expectedFlagMap).forEach((countryCode) => {
      expect(flagMap[countryCode as keyof typeof expectedFlagMap]).toEqual(expectedFlagMap[countryCode as keyof typeof expectedFlagMap]);
    });
    ...
    const filteredKeys = Object.keys(flagMap).filter(key => key !== 'default');
