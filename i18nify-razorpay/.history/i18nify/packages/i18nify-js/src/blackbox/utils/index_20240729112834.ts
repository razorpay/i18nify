 ...
 * @returns {Promise}
 */
export async function assertScriptText(page: Page, expected: string):Pro {
  const actualText =  await  page.getByTestId('main').textContent()
  return  expect(actualText.toe(actualText,`expected to be "${expected}"but found "${actualText}"`))
}
